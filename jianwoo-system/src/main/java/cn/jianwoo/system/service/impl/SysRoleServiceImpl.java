...
import cn.jianwoo.system.domain.SysRoleDept;
import cn.jianwoo.system.domain.SysRoleMenu;
import cn.jianwoo.system.domain.SysUserRole;
import cn.jianwoo.system.mapper.SysRoleDeptMapper;
...
//    @DataScope(projectAlias = "p")
    ...
    public List<SysRole> selectPageRoleList(SysRole role) {
        return roleMapper.selectRolePageList(role);
    }
    ...
    public void checkRoleAllowed(SysRole role) {
        if (StringUtils.isNotNull(role.getRoleId()) && role.isAdmin()) {
            throw new E(MessageUtils.message("role.admin.no.allow.operated"));
        }
    }
    ...
    public void checkRoleDataScope(Long roleId) {
        if (!SysUser.isAdmin(SecurityUtils.getUserId())) {
            ...
            if (StringUtils.isEmpty(roles)) {
                throw new E(MessageUtils.message("no.permission.access.role"));
            }
        }
    }
    ...
    public int insertRole(SysRole role) {
        ...
        if (StrUtil.isBlank(role.getRoleKey())) {
            role.setRoleKey("role_" + sequenceService.queryRoleKeyNextSeq());
        }
        // 新增角色信息
    ...
    public int deleteRoleByIds(Long[] roleIds) {
        for (Long roleId : roleIds) {
            ...
            if (countUserRoleByRoleId(roleId) > 0) {
                throw new E(MessageUtils.message("role.already.assign.can.not.be.deleted", role.getRoleName()));
            }
        }
        // 删除角色与菜单关联
