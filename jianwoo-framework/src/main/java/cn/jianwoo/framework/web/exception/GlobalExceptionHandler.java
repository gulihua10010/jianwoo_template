...
public class GlobalExceptionHandler {
    ...
    public AjaxResult handleBindException(BindException e) {
        ...
        objectName = objectName.replace(Constants.CREATE, "").replace(Constants.EDIT, "").replace(Constants.QUERY, "");
        String msgField = objectName + "." + field;
        String msgFieldDesc = null;
        try {
            msgFieldDesc = MessageUtils.message(msgField);
        ...
        if (StrUtil.isNotBlank(msgFieldDesc)) {
            args.add(0, msgFieldDesc);
        } else {
            args.add(0, field);
        }
